...
namespace ADC
{
    public partial class Receipt : Form
    {
        ...
        private void PrintDoc_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            top += 30;
            e.Graphics.DrawString($"Receipt #: {patientDiagnostic?.Diagnostic?.Id}", normalFont, Brushes.Black, margin, top);
            e.Graphics.DrawString("MO. 8574917390, 8953313751", normalFont, Brushes.Black, margin + 270, top);
            e.Graphics.DrawString($"Date: {patientDiagnostic?.Diagnostic?.DiagnosticDate.ToString("dd-MM-yyyy")}", normalFont, Brushes.Black, margin + 580, top);
            ...
            if (patientDiagnostic != null && patientDiagnostic.Diagnostic != null)
            {
                ...
                if (diagnosticCategory == "XRAY")
                {
                    ...
                    DrawTickMark(checkBoxX, checkBoxY, checkBoxSize, e);
                }

                checkBoxX = margin + 560;
                e.Graphics.DrawString($"E.C.G.:", normalFont, Brushes.Black, margin + 500, top);
                // Draw checkbox rectangle
                e.Graphics.DrawRectangle(Pens.Black, checkBoxX, checkBoxY, checkBoxSize, checkBoxSize);
                // If diagnosticCategory is XRAY, draw tick
                if (diagnosticCategory == "ECG")
                {
                    // Draw a tick mark inside the checkbox
                    DrawTickMark(checkBoxX, checkBoxY, checkBoxSize, e);
                }
        ...
        private void BtnSearchByMob_Click(object sender, EventArgs e)
        {
            ...
            {       
                ...
                {
                    LblReceiptNo.Text = _patientDiagnostic.Diagnostic.Id.ToString();
                    ...
                    ChkXray.Checked = _patientDiagnostic.Diagnostic.DiagnosticCategory == "XRAY";
                    ChkEcg.Checked = _patientDiagnostic.Diagnostic.DiagnosticCategory == "ECG";
        ...
        private void DrawTickMark(int checkBoxX, int checkBoxY, int checkBoxSize, PrintPageEventArgs e)
        {
            // Draw a tick mark inside the checkbox
